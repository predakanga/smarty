    ...
    public function compile($args, $compiler, $parameter, $tag)
    {
        if (strlen($tag) < 6 || substr($tag,-5) != 'close') {
            ...
            foreach ($_attr as $_key => $_value) {
            	...
            	} elseif ($compiler->template->caching && in_array($_key,$compiler->smarty->registered_plugins[Smarty::PLUGIN_BLOCK][$tag][2])) {
            ...
            $this->_open_tag($compiler, $tag, array($_params, $compiler->nocache)); 
            // maybe nocache because of nocache variables or nocache plugin
            $compiler->nocache = !$compiler->smarty->registered_plugins[Smarty::PLUGIN_BLOCK][$tag][1] | $compiler->nocache | $compiler->tag_nocache;
            ...
            list($_params, $compiler->nocache) = $this->_close_tag($compiler, $base_tag); 
