							...
							if (isset($this->template->parent->registered_plugins[$type][$tag][0])) {
								...
								if (is_string($path) && is_file($path)) {
									if ($this->template->caching && ($this->nocache || $this->tag_nocache)) {
										$this->template->required_plugins['nocache'][$tag][$type]['file'] = $path;
										$this->template->required_plugins['nocache'][$tag][$type]['function'] = $function;
									} else {
										$this->template->required_plugins['compiled'][$tag][$type]['file'] = $path;
										$this->template->required_plugins['compiled'][$tag][$type]['function'] = $function;
									}
								}
								if ($type == Smarty::PLUGIN_COMPILER) {
