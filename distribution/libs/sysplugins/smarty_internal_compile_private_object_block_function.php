...
class Smarty_Internal_Compile_Private_Object_Block_Function extends Smarty_Internal_CompileBase {
    ...
    public function compile($args, $compiler, $parameter, $tag, $methode)
    {
        if (strlen($tag) < 5 || substr($tag, -5) != 'close') {
            ...
            $this->_open_tag($compiler, $tag . '->' . $methode, array($_params, $compiler->nocache));
            // maybe nocache because of nocache variables or nocache plugin
            $compiler->nocache = $compiler->nocache | $compiler->tag_nocache;
            // compile code
            $output = "<?php \$_smarty_tpl->smarty->_tag_stack[] = array('{$tag}->{$methode}', {$_params}); \$_block_repeat=true; echo \$_smarty_tpl->smarty->registered_objects['{$tag}'][0]->{$methode}({$_params}, null, \$_smarty_tpl, \$_block_repeat);while (\$_block_repeat) { ob_start();?>";
        } else {
            ...
            list($_params, $compiler->nocache) = $this->_close_tag($compiler, $base_tag . '->' . $methode);
